...
/*****************************************************************************
 * @name       :u16 SPI_WriteByte(u8 Data, u8 Cmd)
 * @date       :2018-08-27 
 * @function   :Write 2 bytes of data using STM32's hardware SPI
 * @parameters :TxData:Data to be written
								Cmd:0-command
								    1-data
 * @retvalue   :Data received by the bus
******************************************************************************/
u16 SPI_WriteByte(u8 Data, u8 Cmd)
{
    u16 Rxdata=0,TxData=0;
		TxData = ((Cmd<<15)|(Data<<7));
    HAL_SPI_TransmitReceive(&SPI5_Handler,(uint8_t*)&TxData,(uint8_t*)&Rxdata,1, 1000);       
...
void SPI5_Init(void)
{
    ...
    SPI5_Handler.Init.DataSize=SPI_DATASIZE_16BIT;       //设置SPI的数据大小:SPI发送接收16位帧结构
